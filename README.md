...
| 类型 | 说明 | 示例 |
|------|------|------|
| 打开网页 | 在浏览器中打开链接 | `https://www.qingup.com` |
| 弹窗提醒 | 弹出系统通知 | `该喝水了！` |
| 系统关机 | 定时关闭电脑 | - |
| 系统重启 | 定时重启电脑 | - |
| 系统休眠 | 定时休眠电脑 | - |
| 锁屏 | 定时锁屏 | - |
| 打开文件夹 | 打开指定目录 | `C:\Documents` |
| 打开文件 | 打开指定文件 | `C:\Documents\note.txt` |
| 执行命令 | 运行 CMD/PowerShell 命令 | `ipconfig /all` |
| 执行脚本 | 运行批处理脚本 | `C:\Scripts\backup.bat` |
| 关闭显示器 | 定时关闭显示器 | - |
| 清空回收站 | 清空系统回收站 | - |
| 注销 | 定时注销当前用户 | - |
| 关闭程序 | 关闭指定进程 | `notepad.exe` |
| 发送UDP消息 | 发送 UDP 数据包 | `127.0.0.1$8080$Hello` |
| 自动截屏 | 定时截屏保存 | `C:\Screenshots`（可选） |
